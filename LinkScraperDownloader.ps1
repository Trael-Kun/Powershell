...
param(
    [string]$Exclude,       #if filename contains this, don't download it
...
foreach ($File in $Files) {
    ...
    if ($Raw) {
        ...
        foreach ($Char in $CharMap) {
            #if the name contains UTF-8 character mapping, replace with corresponding character
            if ($File -like "*$($Char.UTF)*") {
                $FileName = $Filename.replace("$($Char.UTF)","$($Char.Character)")
            }
        }      
    }
