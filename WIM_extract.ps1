...
function Dismount-ISO {
    Write-Host "`nDismounting file `""         -ForegroundColor $Process -NoNewline
    ...
    if (!(Test-Path -Path $IsoMount.DriveLetter)) {     # if successfully ejected
        Write-Host "`nFile `""                 -ForegroundColor $Result      -NoNewline
        ...
        Write-Host "`nUnable to dismount "    -ForegroundColor $ErrorResult -NoNewline
...
if ((Test-Path -Path "$Iso")) {
    ...
    if ($Iso -notlike "*.ISO") {
        ...
        while ($true) {
            Reset-Variables
            $IsoCheck   = Get-ChildItem -Path $Iso -filter *win*.iso |
                Select-Object -Property Name,Fullname
            $IsoCheck | 
                Select-Object @{Name="Item";Expression={$global:i++;$global:i}}, Name -OutVariable menu |
                Format-Table -AutoSize
            $IsoR       = `
                $(Write-Host "Please select file index (number), or type " -ForegroundColor $Prompt     -NoNewline) + `
                $(Write-Host '0'                                           -ForegroundColor $UserInput  -NoNewline) + `
                $(Write-Host ' or '                                        -ForegroundColor $Prompt     -NoNewline) + `
                $(Write-Host 'Q'                                           -ForegroundColor $UserInput  -NoNewline) + `
                $(Write-Host " to quit: "                                  -ForegroundColor $Prompt     -NoNewline) + `
                $(Read-Host)

            if ($IsoR -eq 0 -or $IsoR -eq 'q') {
                Write-Host 'Quitting' -ForegroundColor $Process
                exit 0
            } elseif ($IsoR -gt $IsoGet.Count) {
    ...
    # mount .ISO
    Write-Host "`nMounting `""  -ForegroundColor $Process -NoNewline
    ...
    Write-Host "`nSearching for `"" -ForegroundColor $Process -NoNewline
    ...
    while ($true) {
        ...
        $Index      = $(Write-Host "Please select image index (number), or type " -ForegroundColor $Prompt    -NoNewline) + `
                      $(Write-Host '0'                                            -ForegroundColor $UserInput -NoNewline) + `
                      $(Write-Host ' or '                                         -ForegroundColor $Prompt    -NoNewline) + `
                      $(Write-Host 'Q'                                            -ForegroundColor $UserInput -NoNewline) + `
                      $(Write-Host " to quit: "                                   -ForegroundColor $Prompt    -NoNewline) + `
        ...
        if ($Index -eq '0' -or $Index -eq 'Q') {
            # if 0 or Q entered, dismount & exit loop
            Dismount-ISO
            exit 1
            ...
            if ($IndexSelection -le $IndexTop) {
                ...
                $BadInput       = $false
                Write-Host "`nGetting image details..."
                $WinEd          = Get-WindowsImage -ImagePath "$WimTarget" -Index "$Index"
                Start-Sleep $Sec

                # confirm selection
                Write-Host "`nIndex `""                     -ForegroundColor $Result    -NoNewline
                ...
                if ($WinEd.ImageName[-1] -eq 'N') {
                    Write-Host "`nATTENTION:"               -ForegroundColor $ErrorResult -NoNewline
                ...
                $Confirm = $(Write-Host "`nDo you want to proceed (" -ForegroundColor $Prompt    -NoNewline) + `
                           $(Write-Host "Y"                          -ForegroundColor $UserInput -NoNewline) + `
                           $(Write-Host "/"                          -ForegroundColor $Prompt    -NoNewline) + `
                           $(Write-Host "N"                          -ForegroundColor $UserInput -NoNewline) + `
                           $(Write-Host ")? "                        -ForegroundColor $Prompt    -NoNewline) + `
                           $(Read-Host)
    ...
    if ($WimOut -notlike "*.wim") {
        # if $WimOut isn't .wim, let's name the file
        if (Test-Path $WimOut) {
            # does $WimOut exist?
            ...
            Write-Host "`nOutput path " -ForegroundColor $ErrorResult -NoNewline
            ...
            $Confirm        = $(Write-Host "`nCreate directory `"" -ForegroundColor $Prompt    -NoNewline) + `
        ...
        if (Test-Path $WimPath) {
            ...
            Write-Host " not found." -ForegroundColor $ErrorResult
            Start-Sleep $Sec
            $Confirm        = $(Write-Host 'Create directory "' -ForegroundColor $Prompt    -NoNewline) + `
                              $(Write-Host "$WimPath"           -ForegroundColor $ResultVar -NoNewline) + `
                              $(Write-Host '" ('                -ForegroundColor $Prompt    -NoNewline) + `
                              $(Write-Host "Y"                  -ForegroundColor $UserInput -NoNewline) + `
                              $(Write-Host "/"                  -ForegroundColor $Prompt    -NoNewline) + `
                              $(Write-Host "N"                  -ForegroundColor $UserInput -NoNewline) + `
            ...
            if ($Confirm -eq 'y' -or $Confirm -eq 'yes') {                      # create output directory
                Write-Host "`nCreating " -ForegroundColor $Process -NoNewline
    ...
    # export image
    Write-Host "`nExporting Image..."        -ForegroundColor $Process
    ...
    if ((Test-Path -Path $WimOut)) {
        ...
        Write-Host "`nImage extracted to `""                  -ForegroundColor $Result    -NoNewline
        Write-Host "$WimOut"                                  -ForegroundColor $ResultVar -NoNewline
        Write-Host '"'                                        -ForegroundColor $Result
    ...
    Write-Host "`nUnable to find "        -ForegroundColor $ErrorResult -NoNewline
