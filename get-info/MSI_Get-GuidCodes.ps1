...
<#
 ...
 Modified by Bill 04/11/2024; Tidied formatting
#>
...
DO {
    ...
    if ($Asset -eq "") {
        ...
        $LocalLog = Join-Path -Path $locallogdir -ChildPath $logfile
        $targlog  = Join-Path -Path $targlogdir -ChildPath $logfile
        ...
        if ($PCping -eq $true) {
            ...
            ## Format Output
            Write-Progress -Activity 'Gathering GUIDs' -Status 'Formatting Output'  -PercentComplete 85
            ...
            $packageinfo | Export-Csv $targlog
            if ($PCname -ne $env:ComputerName){
                $packageinfo | Export-Csv $targlog
                $packageinfo | Export-Csv "$locallog"
            } else {
                $packageinfo | Export-Csv "$locallog"
            }
            ...
            if (Test-Path -Path "$targlog") {
                if (Test-Path -path "$locallog") {
                    Write-Host "Logged to " -NoNewline
                    Write-Host "$targlog`r`n" -ForegroundColor Green
                    Write-Host "Logged to " -NoNewline
            ...
            } elseif (Test-Path -Path "$locallog"){
                Write-Host "Logged to " -NoNewline
                Write-Host "$locallog`r`n" -ForegroundColor Green
            } else {
